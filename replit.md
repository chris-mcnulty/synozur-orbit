...
- **Observatory (App Assessments)**: Application/version/assessment registry with shared findings, evidence, and frameworks (`obs_*` tables, `server/routes/observatory.ts`). Six assessment workbenches — Accessibility, Source Code Review, Pen Test Management, Architecture (incl. Azure Well-Architected), Privacy & Compliance, AI Governance — built on checklist-style review items (`server/routes/observatory-modules.ts`, `client/src/pages/app/observatory/`). Pen-test findings wrap shared `obs_findings` rows (CVSS→severity mapping); AI Governance init is blocked unless the application is flagged `aiEnabled`; scanner/repo integrations are documented design-only interfaces in `server/services/observatory-scanners.ts`. V1 insights layer (`server/routes/observatory-insights.ts`): weighted readiness engine with hard certification blockers (`server/services/observatory-readiness.ts`), executive dashboard (8 KPIs + alerts), async report generation — 5 report types, 202-and-poll via the job queue, PDF via the shared pipeline (`server/services/observatory-report-service.ts`), and a VPAT assistant (WCAG 2.2 + Section 508 worksheet, AI-drafted remarks, mandatory draft disclaimer).
