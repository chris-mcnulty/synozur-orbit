...
- **Campaigns (Social)**: Containers for content assets, social accounts, and generated social posts, with automatic hashtag merging. Includes manual post creation flow (Create Post button + compose dialog) allowing users to paste/type their own content and distribute across selected social accounts with optional AI polish, brand image, and scheduled date. Manual posts use the same `generatedPosts` table and follow the same approve/edit/reject workflow. Platform-specific text adaptation applied (e.g. Twitter 280 char truncation).
