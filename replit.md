...
- **Authorization**: Role hierarchy (Global Admin > Domain Admin > Analyst > Standard User > Consultant). Customer-facing tiers: Domain Admin (full tenant admin), Analyst (content authoring + intelligence building, no user/billing management), Standard User (read-only). `hasAdminAccess()` gates user/billing/SSO/tenant routes; `hasContentAccess()` gates content-authoring routes and includes Analyst.
...
- **Relationship Reports** (Trial/Pro/Enterprise/Unlimited, `relationshipReports` feature key): On-demand 12-month engagement plans for any tracked competitor, written from the baseline company's perspective. Covers cooperate/compete/sell-to/steer-clear/observe postures with quarter-by-quarter actions. Anchored in baseline messaging framework + GTM plan + uploaded grounding docs (official docs win on conflict). Markdown/PDF/Word export, version history, SPE-archived. Routes: `/api/relationship-reports/*`. Page: `/app/relationship-reports`. Service: `server/services/relationship-report-service.ts`. PDF: `generateRelationshipReportPdf` in `server/services/pdf-generator.ts`.
