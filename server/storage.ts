...
export class DatabaseStorage implements IStorage {
  ...
  async getCompetitorsByOrganizationId(organizationId: string): Promise<Competitor[]> {
    return db.select().from(competitors).where(eq(competitors.organizationId, organizationId));
  }
  ...
  private _extractOrgDataFromCompetitor(comp: Competitor): Partial<InsertOrganization> {
    return {
      ...
      // jsonb columns read back as `unknown`; the target insert type expects
      // Drizzle's Json shape. The data is identical jsonb passing straight
      // through, so a cast is safe here.
      crawlData: comp.crawlData as any,
      ...
      linkedInEngagement: comp.linkedInEngagement as any,
      instagramEngagement: comp.instagramEngagement as any,
      twitterEngagement: comp.twitterEngagement as any,
      blogSnapshot: comp.blogSnapshot as any,
  ...
  private _extractOrgDataFromProfile(profile: CompanyProfile): Partial<InsertOrganization> {
    return {
      ...
      // jsonb columns read back as `unknown`; cast straight through to the
      // Json insert type (identical jsonb data).
      ...
      twitterEngagement: profile.twitterEngagement as any,
