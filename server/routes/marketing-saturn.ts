  ...
  app.put("/api/campaigns/:campaignId/generated-posts/:postId", async (req, res) => {
    ...
    try {
      ...
      const [campaign] = await db.select().from(campaigns)
        .where(and(eq(campaigns.id, req.params.campaignId), eq(campaigns.tenantDomain, ctx.tenantDomain)));
      if (!campaign) return res.status(404).json({ error: "Campaign not found" });
      const { editedContent, status, overrideImageUrl, overrideBrandAssetId, scheduledDate, hashtags, linkUrl, linkLabel, publishedUrl, socialAccountId, deliveryMode, publishedAt: bodyPublishedAt } = req.body;
      ...
      if (status !== undefined) {
        ...
        if (status === "published") {
          ...
          if (existing && !existing.publishedAt) {
            updateFields.publishedAt = bodyPublishedAt ? new Date(bodyPublishedAt) : new Date();
          }
        }
      }
...
- Suitable for customer communications, follow-ups, and account management.`,
    ...
    let platformInstruction = platformInstructions[platformKey] || platformInstructions["outlook"];
    ...
    // Resolve LOGO_RULE in platformInstruction based on whether a logo URL is available
    const logoUrl = companyProfile?.logoUrl || logoAsset?.fileUrl || logoAsset?.url || (tenantRow as any)?.logoUrl || "";
    const resolvedLogoRule = logoUrl
