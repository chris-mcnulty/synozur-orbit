...
- **Refresh-token rotation reuse:** X refresh tokens are single-use; reusing one (worker tick's stale in-memory account after an earlier post refreshed, or concurrent manual Retry) makes X revoke the whole grant → "Value passed for the token was invalid" even right after reconnect. Worker must re-fetch the account row before each publish; publisher serializes publishes per account and refreshes 60s before expiry.
