...
> **Status (in progress):** ✅ `prospector-core` (ICP scoring, disqualifiers, geo/industry match, criteria builder) + unit tests; ✅ `outreach-interview-core` (step graph, goal classification, cadence archetypes incl. event-anchored back-dating, targeting normalization) + unit tests; ✅ `outreach-interview-service` (creates campaign + default cadence from the brief, in one txn); ✅ `prospector-service` (deterministic score + grounded AI dossier, advances `new→researched`/`dormant`); ✅ routes: campaigns CRUD, prospect add/list, metered `POST /prospects/:id/research`. ✅ **UI**: the interview wizard (`outreach-interview.tsx`), campaign list with readiness banner (`outreach-campaigns.tsx`), and campaign detail with prospects table + add-prospect + research action + dossier dialog (`campaign-detail.tsx`); wired into `App.tsx` + `areaNavigation.ts` ("Outreach" under Sales). ✅ **HubSpot bulk import** (pull existing contacts → prospects, deduped). ✅ **Per-user voice-DNA extraction** — `outbound-voice-service` reads the seller's Outlook Sent Items (delegated Graph) and extracts a personal voice profile (tone, style, preferred/forbidden phrases, sample snippets) into `social_account_voice_profiles` (now nullable `socialAccountId` + `ownerUserId`, migration `0045`); exposed at `/api/sales-outreach/voice[/extract]`, surfaced on the settings page, fed into readiness, and auto-selected as a new campaign's voice. **Phase 1 complete.**
...
> **Status (in progress):** ✅ `compliance-core` (AI-cliché/banned-phrase scan seeded from the brand voice guidelines, suppression + self-email hard blocks, CAN-SPAM checks) + unit tests; ✅ `outreach-composer-core` (per-channel prompt assembly, parsing, length guardrails) + unit tests; ✅ `outreach-composer-service` (grounds in dossier + voice + strategic context + step resource, scans every draft, writes `draft_pending_approval`); ✅ `outlook-draft-service` (creates a draft in the seller's mailbox via the delegated Graph token, clear errors on missing consent/scope); ✅ **per-seller `Mail.ReadWrite` consent flow** (`/api/sales-outreach/mailbox/consent-url` + status; refresh now uses the user's *granted* scopes so it never drops Planner; reuses the shared Graph callback) with a **settings page** (mailbox connect + circuit-breaker caps); ✅ routes: `compose`, `touches` list, `PATCH` edit + re-scan, `approve` (→ Outlook draft, honors global pause + compliance hard blocks, writes the send ledger, **best-effort HubSpot contact upsert + note logging**); ✅ **two-way HubSpot** — import existing contacts as prospects (`/import-hubspot`, deduped) and push prospects on approve / on-demand (`/sync-hubspot`, adds `crm.objects.contacts.write`); ✅ UI: compose → review → **Approve → Outlook**, **Import from HubSpot**, and the settings page. **Remaining:** LinkedIn send (Phase 4 provider) and the resource-attach UI (the `injectAtStep` resource model + compose-time injection already work server-side).
...
> **Status (in progress):** ✅ `cadence-core` (state machine, timing incl. business-day shift + event-anchored back-dating, due-step computation, **master + per-channel + per-domain cap math**, reply-rate floor) + unit tests; ✅ `cadence-service` (ledger-counted **circuit-breaker enforcement** now wired into approve — fails closed; DB-driven `tickCadence` advances due steps `awaiting_reply → cadence_step_due`, exhausts sequences → `dormant`, auto-pauses campaigns under the reply floor; `getOutreachSummary` rollup); ✅ routes: `summary`, `cadence/tick`, `mark-replied`; ✅ UI: **Active Outreach widget** on Sales home, plus **Refresh cadence** + **Mark replied** on the campaign page. **Remaining:** scheduled auto-tick (currently manual/route) and Graph-based send/reply *auto*-detection (needs a live mailbox; today approve → `awaiting_reply` and reply is marked manually).
> **Update — Phase 3 live bits done:** ✅ **scheduled auto-tick** — hourly `runOutreachCadenceJob` sweep in `scheduled-jobs.ts` (per active, plan-eligible tenant) running `runCadenceForTenant`; ✅ **Graph send/reply auto-detection** — `detectMailboxActivity` confirms approved email touches actually sent (draft left Drafts → `sent`) and detects inbound replies from prospects → `replied`, best-effort per seller's delegated token; also runs on the manual "Refresh cadence" for the current seller. Manual `mark-replied` remains as a fallback.

**Phase 4 — LinkedIn provider + performance loop.** Wire the LinkedIn **MCP client** for research/messaging; close the loop — reply/meeting rates and HubSpot deal attribution feed back into ICP scoring (mirrors the marketing performance-analyst).
> **Status (in progress):** ✅ `linkedin-provider-core` (pure backend selector: direct OAuth preferred for posting once approved, MCP for messaging, copy-assist otherwise) + unit tests; ✅ `linkedin-provider` service (`getLinkedInCapabilities`, `sendLinkedInMessage` — MCP-gated, falls back cleanly) wired into approve so LinkedIn touches send via MCP when available or stay copy-assist; ✅ `outreach-performance-core` (conversion-first: funnel, reply rate, **per-ICP-signal lift** that feeds targeting, recommendations) + unit tests; ✅ `outreach-performance-service` + `GET /campaigns/:id/performance` + `GET /linkedin/capabilities`; ✅ UI: a **Performance** card on the campaign page (reply rate, what's converting, recommendations). **Remaining (live-integration):** connect an actual LinkedIn MCP server (then messaging/posting light up with no caller changes), and HubSpot deal-stage attribution feeding the report.
