...
> **Status (in progress):** ✅ `compliance-core` (AI-cliché/banned-phrase scan seeded from the brand voice guidelines, suppression + self-email hard blocks, CAN-SPAM checks) + unit tests; ✅ `outreach-composer-core` (per-channel prompt assembly, parsing, length guardrails) + unit tests; ✅ `outreach-composer-service` (grounds in dossier + voice + strategic context + step resource, scans every draft, writes `draft_pending_approval`); ✅ `outlook-draft-service` (creates a draft in the seller's mailbox via the delegated Graph token, clear errors on missing consent/scope); ✅ routes: `compose`, `touches` list, `PATCH` edit + re-scan, `approve` (→ Outlook draft, honors global pause + compliance hard blocks, writes the send ledger); ✅ UI: compose → review (editable subject/body + compliance flags) → **Approve → Outlook** in the campaign detail page. **Remaining:** the per-seller incremental `Mail.ReadWrite` consent flow (so approve actually lands a draft live), LinkedIn send (Phase 4 provider), HubSpot activity logging on approve, and resource-attach UI.
